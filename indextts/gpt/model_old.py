...
class UnifiedVoice(nn.Module):
    ...
    def init_vllm(self):
        print(f"Loading vLLM engine with model: {self.vllm_save_dir}, "
    ...
    def post_init_gpt2_config(self, use_deepspeed=False, kv_cache=False, half=False):
        ...
        if self.initialize_vllm:
            ...
            try:
                self.init_vllm()
                print(f"vLLM engine initialized from {self.vllm_save_dir}")
    ...
    async def inference_speech_vllm(
        ...
        **kwargs # Catch any other unused kwargs
        ...
        with torch.no_grad():
            conds = self.get_conditioning(speech_conditioning_latent, cond_mel_lengths)
